...
var LabelRels = struct {
	...
	Article:           "Article",
	User:              "User",
	ClickbaitKeywords: "ClickbaitKeywords",
}
...
type labelR struct {
	Article           *Article
	User              *User
	ClickbaitKeywords ClickbaitKeywordSlice
}
...
// ClickbaitKeywords retrieves all the clickbait_keyword's ClickbaitKeywords with an executor.
func (o *Label) ClickbaitKeywords(mods ...qm.QueryMod) clickbaitKeywordQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"clickbait_keywords\".\"label_id\"=?", o.ID),
	)

	query := ClickbaitKeywords(queryMods...)
	queries.SetFrom(query.Query, "\"clickbait_keywords\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"clickbait_keywords\".*"})
	}
...
// LoadClickbaitKeywords allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (labelL) LoadClickbaitKeywords(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLabel interface{}, mods queries.Applicator) error {
	var slice []*Label
	var object *Label
	...
	query := NewQuery(qm.From(`clickbait_keywords`), qm.WhereIn(`clickbait_keywords.label_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load clickbait_keywords")
	}

	var resultSlice []*ClickbaitKeyword
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice clickbait_keywords")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on clickbait_keywords")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for clickbait_keywords")
	}
	...
	if singular {
		object.R.ClickbaitKeywords = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &clickbaitKeywordR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.LabelID {
				local.R.ClickbaitKeywords = append(local.R.ClickbaitKeywords, foreign)
				if foreign.R == nil {
					foreign.R = &clickbaitKeywordR{}
				}
...
// AddClickbaitKeywords adds the given related objects to the existing relationships
// of the label, optionally inserting them as new records.
// Appends related to o.R.ClickbaitKeywords.
// Sets related.R.Label appropriately.
func (o *Label) AddClickbaitKeywords(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ClickbaitKeyword) error {
	var err error
	...
	if o.R == nil {
		o.R = &labelR{
			ClickbaitKeywords: related,
		}
	} else {
		o.R.ClickbaitKeywords = append(o.R.ClickbaitKeywords, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &clickbaitKeywordR{
				Label: o,
			}
		} else {
			rel.R.Label = o
